...
async function loadGlbTemplate(stepUrl: string, label: string): Promise<CadMeshTemplate> {
  ...
  const loading = (async () => {
    ...
    gltf.scene.traverse((object) => { if (!sourceMesh && object instanceof THREE.Mesh) sourceMesh = object })
    ...
    const material = sourceMaterial instanceof THREE.MeshStandardMaterial ? sourceMaterial.clone() : new THREE.MeshStandardMaterial({ color: 0x777777, metalness: 0.15, roughness: 0.7 })
    geometry.computeBoundingBox(); geometry.computeBoundingSphere()
    console.log('[GLB cache] ready', label, glbUrl)
    return { geometry, material }
  })()
  glbCache.set(key, loading)
  try { return await loading } catch (error) { glbCache.delete(key); throw error }
}

function createMesh(template: CadMeshTemplate, name: string) { const mesh = new THREE.Mesh(template.geometry, template.material); mesh.name = name; mesh.scale.setScalar(0.01); return mesh }
...
export function OpenGeometryCadScene({ width, height, depth, railCount, plinthHeight }: OpenGeometryCadSceneProps) {
  ...
  useEffect(() => { parametersRef.current = { width, height, depth, railCount, plinthHeight }; applyAssemblyRef.current?.() }, [width, height, depth, railCount, plinthHeight])
  useEffect(() => {
    const container = containerRef.current; if (!container) return
    let disposed = false; let loadVersion = 0; let importedPlinth: THREE.Mesh | null = null; let importedCabinet: THREE.Mesh | null = null; let applyTimer: ReturnType<typeof setTimeout> | null = null
    const scene = new THREE.Scene(); scene.background = new THREE.Color('#101010')
    const camera = new THREE.PerspectiveCamera(45, 1, 0.1, 100); camera.position.set(4, 3.5, 4)
    const renderer = new THREE.WebGLRenderer({ antialias: true }); renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2)); renderer.setSize(container.clientWidth, container.clientHeight); container.appendChild(renderer.domElement)
    const controls = new OrbitControls(camera, renderer.domElement); controls.enableDamping = true; controls.target.set(0, 1.5, 0); controls.update()
    scene.add(new THREE.HemisphereLight(0xffffff, 0x444444, 2)); const directional = new THREE.DirectionalLight(0xffffff, 2); directional.position.set(5, 10, 7); scene.add(directional); scene.add(new THREE.GridHelper(12, 24, 0x444444, 0x222222))
    ...
    applyAssemblyRef.current = scheduleAssembly; scheduleAssembly()
    const resize = () => { const aspect = container.clientWidth / Math.max(container.clientHeight, 1); camera.aspect = aspect; camera.updateProjectionMatrix(); renderer.setSize(container.clientWidth, container.clientHeight) }
    ...
    return () => { disposed = true; loadVersion += 1; applyAssemblyRef.current = null; if (applyTimer) clearTimeout(applyTimer); cancelAnimationFrame(frame); resizeObserver.disconnect(); controls.dispose(); renderer.dispose(); container.removeChild(renderer.domElement) }
  }, [])
