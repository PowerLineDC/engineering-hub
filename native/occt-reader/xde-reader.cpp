...
void writeObj(const std::filesystem::path& path,const TopoDS_Shape& shape) {
    ...
    for(TopExp_Explorer it(shape,TopAbs_FACE);it.More();it.Next()) {
        ...
        for(int n=1;n<=tri->NbTriangles();++n){ int a,b,c; tri->Triangle(n).Get(a,b,c); if(rev) std::swap(b,c); out<<"f "<<offset+a-1<<" "<<offset+b-1<<" "<<offset+c-1<<"\n"; }
        offset+=nodes;
    }
}
...
int run(const std::filesystem::path& source,const std::filesystem::path& jsonPath,const std::filesystem::path& componentDir) {
    ...
    for(size_t i=0;i<leaves.size();++i) {
        ...
        std::string id="component-"+std::to_string(i+1); auto obj=componentDir/(id+".obj"); writeObj(obj,shape);
        out<<"    {\"id\":\""<<id<<"\",\"name\":\""<<jsonEscape(name)<<"\",\"labelTag\":"<<label.Tag()<<",\"position\":["<<info.x<<","<<info.y<<","<<info.z<<"],\"size\":["<<info.sx<<","<<info.sy<<","<<info.sz<<"],\"volume\":"<<info.volume<<",\"modelUrl\":\"/api/cad/xde-component?file="<<obj.filename().string()<<"\"}"<<(i+1<leaves.size()?",":"")<<"\n";
    }
    out<<"  ]\n}\n"; return 0;
}
}
