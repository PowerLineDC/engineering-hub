...
function postCandidates(objects, box) {
  ...
  const candidates = objects.filter(o => {
    const b=o.bbox
    return b.dz > box.dz*0.35 && b.dz > Math.max(b.dx,b.dy)*3
  })
  const result={}
  for (const [name,x,y] of corners) {
    const best=candidates.filter(o => !Object.values(result).some(r=>r.index===o.index)).sort((a,b)=>Math.hypot(a.center.x-x,a.center.y-y)-Math.hypot(b.center.x-x,b.center.y-y))[0]
...
async function main() {
  ...
  const valid=source.filter(a=>validBBox(a.assemblyBBox)&&Array.isArray(a.objects)&&a.objects.length)
  const assemblies=[]
  for(const a of valid){
    ...
    const expectedPost=finite(d.H)?d.H-FIXED_Z:null
    const expectedX=finite(d.W)?d.W-2*POST:null
    const expectedY=finite(d.D)?d.D-2*POST:null
    assemblies.push({
      ...
      postLengthMm:round(postLength), expectedPostLengthMm:round(expectedPost), postLengthDeltaMm:round(postLength-expectedPost),
      spacingXmm:round(spacingX), expectedSpacingXmm:round(expectedX), spacingXDeltaMm:round(spacingX-expectedX),
      spacingYmm:round(spacingY), expectedSpacingYmm:round(expectedY), spacingYDeltaMm:round(spacingY-expectedY),
      positioningRule:{X1:uniqueX[0]??null,X2:uniqueX[1]??null,Y1:uniqueY[0]??null,Y2:uniqueY[1]??null},
    })
  }
  const result={
    schemaVersion:1,generatedAtUtc:new Date().toISOString(),
