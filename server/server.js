  ...
  if (req.method === 'POST' && req.url === '/api/estimate') {
    ...
    req.on('end', () => {
      try {
        ...
        if (!input || typeof input !== 'object' || typeof input.id !== 'string') return send(res, 400, { error: 'Некорректные данные' })
...
server.listen(PORT, HOST, () => console.log(`Engineering Hub local server: http://localhost:${PORT}`))
